/**
 * GuideOverlay.jsx — Figma-style alignment guides + spacing measurements
 *
...
const SNAP_COLOR = '#6ee7b7'    // soft teal – alignment snap lines
const MEAS_COLOR = '#ff4d6d'    // Figma-red – spacing measurements
const MEAS_BG    = 'rgba(255,77,109,0.13)'

// Small measurement badge rendered as SVG (avoids DOM overlay z-index issues)
function MeasBadge({ cx, cy, val }) {
...
export default function GuideOverlay({ guides, zoom, canvasW, canvasH }) {
  ...
  const cvx = cx => ox + cx * zoom
  const cvy = cy => oy + cy * zoom
    ...
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 50 }}>
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', overflow: 'visible' }}>
        ...
        {nearbyDistances.map((m, i) => {
          ...
          if (m.type === 'h') {
            const x1 = cvx(m.from), x2 = cvx(m.to), y = cvy(m.midY)
            return (
              <g key={`nm${i}`}>
            ...
            const y1 = cvy(m.from), y2 = cvy(m.to), x = cvx(m.midX)
            return (
              <g key={`nm${i}`}>
