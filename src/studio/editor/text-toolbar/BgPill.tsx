...
const BgPill = memo(function BgPill({ apply }: BgPillProps): JSX.Element {
  ...
  return (
    <Pill modified={hasBg} mixed={mixed}>
      ...
      >
        {/* Color dot: gradient when mixed, actual color when set */}
        {mixed
          ? <div style={{ width: 12, height: 12, borderRadius: '50%', flexShrink: 0,
              background: 'linear-gradient(135deg, #FFD93D 50%, #4488FF 50%)',
              border: '1px solid rgba(255,255,255,0.2)' }}/>
          : <div style={{ width: 12, height: 12, borderRadius: '50%', flexShrink: 0,
              background: hasBg ? (textBackgroundColor as string) : 'rgba(255,255,255,0.18)',
              border: '1px solid rgba(255,255,255,0.2)' }}/>
        }
        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', fontFamily: 'sans-serif' }}>BG</span>
        <svg width="7" height="5" viewBox="0 0 7 5" fill="none">
          <path d="M1 1l2.5 3L6 1" stroke="rgba(255,255,255,0.3)" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </button>
      ...
      {open && (
        <Tray align="right" style={{ width: 172 }}>
          <p style={{ margin: '0 0 7px', fontSize: 9, color: 'rgba(255,255,255,0.28)', fontFamily: 'sans-serif', textTransform: 'uppercase', letterSpacing: '.07em' }}>Highlight</p>
            ...
            {BG_SWATCHES.map(c => (
              <button key={c} onMouseDown={(e: MouseEvent<HTMLButtonElement>) => e.preventDefault()}
                ...
                style={{
                  ...
                  outline: ((textBackgroundColor as string) || '').toLowerCase() === c.toLowerCase() ? '2px solid #fff' : '2px solid transparent',
          ...
          <button onMouseDown={(e: MouseEvent<HTMLButtonElement>) => e.preventDefault()}
            ...
            style={{
              width: '100%', padding: '5px 7px', border: '1px dashed rgba(255,255,255,0.15)',
              borderRadius: 7, background: 'transparent', color: 'rgba(255,255,255,0.4)',
